...
STEP 2: Fetch jira.getIssue(issue_key="**Story Templete JIRA ID**", expand="renderedFields", fields="*all") → template_structure (sections, headings, format).
...
• Confluence page URLs containing "pageId=" or "pages/viewpage.action?pageId=" 
  - Example: your-company.atlassian.net/wiki/pages/viewpage.action?pageId=xxxxxxxx
  - Example: your-company.atlassian.net/wiki/spaces/Project ID/pages/xxxxxxxxxx/
...
STEP 7: Generate Stories with Senior BA Practices (Project: XXXX, Type: Story)
TEAM Selection : Team Name XXXX (REST API, testing, integration) | Team Name YYYY (Offerings, Catalog, DB, invoice, Opcode, FLIST, pricing/account)
...
Analyze architecture_analysis to determine target team(s):
• **Team Name ONLY**: If story contains ONLY rest APIs, integration, integration testing → Assign to Team name XXXX
• **Team Name ONLY**: If story contains ONLY Offerings, Catalog, DB, invoice, Opcode, FLIST, pricing/account → Assign to Team Name YYYY
• **SPLIT INTO SEPARATE STORIES**: If work contains BOTH categories → Create SEPARATE stories:
  - Story A (Team Name XXXX): BRM internal logic, catalog, database, pricing, offerings
  - Story B (Team Name YYYY): REST API integration, SFCC integration, end-to-end testing
  - Example: "Implement BRM Service Creation Logic" (Team Name YYYY) + "End-to-End Integration Testing via RSM API" (Team Name XXXX)
→ **Set customfield_12725 (Development Team)** to single team value for each story: "Team Name XXXX" OR "Team Name YYYY" (never both)

STORY BREAKDOWN STRATEGY (Senior BA Approach):
• Break by TEAM BOUNDARIES first (never mix Team Name YYYY and Team Name XXXX work in same story)
...
• BRM internal logic (Team Name YYYY) = separate from RSM API integration (Team Name XXXX)
...
• <determined_dev_team> = "Team Name XXXX" | "Team Name YYYY" (MUST be exactly one team per story)
